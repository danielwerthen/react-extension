...
function _extendTag(tagName: string): React.FunctionComponent {
  const Component = reactModule.forwardRef((props: unknown, ref: unknown) => {
    ...
    const newProps = extender ? extender(props) : props;
    return reactCreateElement(tagName, { ...newProps, ref });
  });
  if (process.env.NODE_ENV !== "production") {
    Component.displayName = `Extended(${tagName})`;
  }
  return Component;
}

export default function extendTag(tagName: string): React.FunctionComponent {
